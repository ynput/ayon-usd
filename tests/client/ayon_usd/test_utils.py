...
def test_validate_file_checksum(file_info, tmp_path):
    ...
    assert utils.validate_file_checksum(
        str(file_path), file_info["checksum"], file_info["checksum_algorithm"]
    )
...
def test_file_info_endpoint(
    printer_session, installed_addon, ayon_server_session, ayon_connection_env
):
